...
{
	WiFi_Controlflag = 0;	//遥控器控制时失能App控制
	if(NRF_RX_DATA[11] != 0xa5)	//验证校验码是否为0xa5
	{
		return;
	}
	if(NRF_RX_DATA[0]&0x01)	//当数据包是由遥控器的ADC采样完成时触发发送
	{
...
void Button_command(uint8_t Button)
{
	...
	if((PreButton&0x02) != (Button&0x02))
	{
		...
		{
			WiFi_Switch(ENABLE);	//开启WiFi
			Run_flag = 0;
			SENSER_FLAG_SET(WiFi_ONOFF);	
			WiFi_LEDflag = 1;
		}
		else	//关闭WiFi
		{
	...
	//遥控器K3按键	模式选择
	if((PreButton&0x04) != (Button&0x04))
	{
		if(Button&0x04)		//无头模式
		{
			SENSER_FLAG_RESET(FLY_MODE); //无头模式
		}else{				//有头模式
			SENSER_FLAG_SET(FLY_MODE); //有头模式
		}
	}
	//遥控器K4按键	传感器校准
	if((PreButton&0x08) != (Button&0x08))
	{
		if(Button&0x08)//陀螺仪加速度计校准
		{
			if(!Airplane_Enable)
...
void WiFi_Data_ReceiveAnalysis(uint8_t *buff, uint8_t cnt)
{
	...
	else if(buff[0] == 0xAA && buff[1] == 0xBB)	//判断帧头
	{
		if(buff[2] == 0x01)	//数据帧
		{
			if(buff[3] == 0x08)	//数据长度
			{
				RC_Control.THROTTLE = (((uint16_t)buff[4]<<8|buff[5])-1000);
